package jp.co.saisk._28_itemprocessor_script;
...
public class ScriptProcessorJob {
	...
	public static void main(String[] args) {
		...
		System.exit(SpringApplication.exit(SpringApplication.run(ScriptProcessorJob.class, args)));
	}
    ...
    public ScriptItemProcessor<User, User> scriptItemProcessor(){
        ScriptItemProcessor<User, User> processor = new ScriptItemProcessor<User, User>();
        //加载js文件， 执行js中转换逻辑
	...
	public Job job() throws Exception {
		return new JobBuilder("script-processor-job", jobRepository) // 创建一个 Job 构建器
				.start(step())
				.build();
	}
